...
$resContactos = $serviciosReferencias->traerGrupoespecificaiones();

$resContactosCountries = $serviciosReferencias->traerCategoriasespecificacionPorCategoria($id);
...
while ($rowFS = mysql_fetch_array($resContactos)) {
	...
	if (mysql_num_rows($resContactosCountries)>0) {
		foreach ($arrayFS as $item) {
			if (stripslashes($item['refgrupoespecificaciones']) == $rowFS[0]) {
				$check = 'checked';	
